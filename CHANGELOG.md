...
- **Deploy no longer breaks when `dl.rockylinux.org` 403s.** Closes #11. The catalog `Distro` entry for Rocky 9 now lists three equivalent mirror URLs (`dl.rockylinux.org`, `download.rockylinux.org`, `mirror.rackspace.com`); on deploy the api container HEAD-probes them in order and passes the first 2xx URL to terraform. A transient `host_not_allowed` 403 from Rocky's primary mirror (the failure mode reported in the wild) silently falls through to the next mirror instead of breaking the deploy. All three serve the same SHA256-pinned bytes. The custom-URL escape hatch is unchanged. Caveat: the probe runs from the api container's network, not from Proxmox — for split-network topologies (operator host on a separate VLAN), all-fail logs a warning and falls through to the first URL rather than blocking.
- **Operators can relocate `tf-state` and log roots without recompiling.** Closes #15. The two paths previously hardcoded into `deployments.Executor.TfStateRoot` and `{deployments,apps}.Executor.LogRoot` now read from `BANDOLIER_TF_STATE_ROOT` and `BANDOLIER_LOG_ROOT` (defaults match the prior values). The single `BANDOLIER_LOG_ROOT` value is shared by both executors so they can no longer drift. All five `BANDOLIER_*` runtime overrides now go through one `envWithDefault` helper, and the new vars are documented in a new **Configuration** section in the README.
