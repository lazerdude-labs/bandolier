...
A security-hardening release driven by a full-project baseline review. It closes the four highest-priority findings: the api↔vault channel ran in plaintext (constitution Principle I requires mTLS), no service container dropped Linux capabilities or forbade privilege escalation, ingress hostnames reached `helm --set` unvalidated (the same values-injection class the v0.1.17 StorageClass fix closed), and operator-cancelled operations left no terminal audit row. No feature or API-surface change. Pull `ghcr.io/lazerdude-labs/bandolier/{api,ui,vault-agent,tls-init}:0.1.18` (or `:0.1` / `:latest`) to upgrade.

**Upgrade note (mTLS cutover).** On `docker compose up` with the new images, `tls-init` mints the internal CA + Vault server/client certs into the existing `tls` volume (idempotent — your UI cert is untouched), and Vault restarts with a TLS listener requiring client certs. The existing initialized `vault-data` and unseal keys persist; vault-agent and the api reconnect over mTLS automatically. This is a rolling, in-place upgrade with no manual re-init, but it does change Vault's transport — validate the stack comes back healthy (`docker compose ps`, `bandolier-vault` healthy, the api reaches `api listening`) after the pull.

### Security

- **mTLS on the api↔vault channel.** Vault's listener previously ran `tls_disable = 1` and the api + vault-agent connected over `http://vault:8200`; anything on the compose bridge could read the AppRole `secret_id`, client token, and all cluster KV in transit. `tls-init` now mints an internal CA plus a Vault server cert (SAN `vault`) and client certs for the api and vault-agent. Vault's listener runs `tls_require_and_verify_client_cert = true`; the vault-agent routes every API call through a client-cert `vcurl` wrapper, and the api configures its Vault client with `cfg.ConfigureTLS` (CA + client cert/key from `BANDOLIER_VAULT_*` env). Verified end-to-end on an isolated instance, including a negative control proving the listener rejects certless clients (`tls: certificate required`). Leaf private keys live at `0644` on the internal, never-host-exposed, read-only-mounted `tls` volume (CA key stays `0600`) — under the single-trusted-operator threat model the volume/exec boundary, not file mode, is what protects them.
- **Stack container hardening.** Every service now runs `cap_drop: [ALL]`. `tls-init`, `vault-agent`, and `ui` additionally run `read_only` rootfs (with tmpfs scratch) and `no-new-privileges`; `ui` re-adds only `CHOWN/SETUID/SETGID/NET_BIND_SERVICE`. `vault` keeps the minimal capability set its official entrypoint needs (`CHOWN/DAC_OVERRIDE/SETUID/SETGID/SETFCAP/IPC_LOCK`, preserving mlock so secrets stay out of swap). The `api` drops all caps and forbids privilege escalation; its `read_only`/non-root conversion is deferred pending live-lab validation of the terraform/ansible/helm/kubectl/ssh subprocess write paths (tracked in `docs/reviews/2026-06-02-baseline-code-review.md`).
- **Ingress hostnames validated before helm.** Operator-supplied `hostname` (in `/apps/install`, `/apps/{release}/upgrade`, and bundle choices) reached a `helm --set <path>=<hostname>` argument with no validation. helm splits `--set` on commas within a single argument, so a value like `app.example.com,admin.password=evil` could smuggle an arbitrary chart value past the ingress field. Hostnames are now checked against an RFC 1123 allowlist (≤253 chars) and rejected with a 400, closing the same injection class the v0.1.17 StorageClass validation closed.

### Fixed

- **Cancelled operations now persist a terminal audit row.** `audit.Write` rejected the `cancelled` outcome, so the terminal audit entry for any operator-cancelled deploy/destroy/upgrade was silently dropped (callers discard the write error), leaving a dangling `started` row with no pair. `cancelled` is now an accepted outcome, restoring audit completeness (actor/target/outcome/timestamp for every state-changing action).
